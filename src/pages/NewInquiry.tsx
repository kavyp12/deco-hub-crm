...
const inquirySchema = z.object({
  ...
  sales_person_id: z.string().uuid('Please select a sales person'),
  expected_final_date: z.string().optional(),
  product_category: z.enum(['drapes', 'blinds', 'rugs'], {
...
const NewInquiry: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    sales_person_id: '',
    expected_final_date: '',
    product_category: '' as 'drapes' | 'blinds' | 'rugs' | '',
  });
  ...
  useEffect(() => {
    ...
    const fetchSalesPeople = async () => {
      const { data, error } = await supabase
        .from('user_roles')
        .select('user_id, profiles!user_roles_user_id_fkey(id, name)')
        .eq('role', 'sales');
      ...
      const salesList = data
        ...
        .filter((p): p is SalesPerson => p.id && p.name);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('inquiries').insert({
        ...
        architect_id_name: formData.architect_id_name || null,
        mobile_number: formData.mobile_number,
        inquiry_date: formData.inquiry_date,
        address: formData.address,
        sales_person_id: formData.sales_person_id,
        expected_final_date: formData.expected_final_date || null,
        product_category: formData.product_category as 'drapes' | 'blinds' | 'rugs',
        created_by: user?.id,
      });
      ...
      toast({
        ...
        description: `Inquiry ${inquiryNumber} has been created successfully.`,
      });
    ...
    <DashboardLayout>
      <div className="max-w-3xl animate-fade-in">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">New Inquiry</h1>
          ...
          <div className="card-premium p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="architect_id_name"
                  name="architect_id_name"
                  placeholder="Enter architect or ID name"
                  value={formData.architect_id_name}
                  onChange={handleChange}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={handleChange}
                  className={errors.inquiry_date ? 'border-destructive' : ''}
                />
                {errors.inquiry_date && (
                  <p className="text-sm text-destructive">{errors.inquiry_date}</p>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => handleSelectChange('sales_person_id', value)}
                >
                ...
                {errors.sales_person_id && (
                  <p className="text-sm text-destructive">{errors.sales_person_id}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="expected_final_date"
                  name="expected_final_date"
                  type="date"
                  value={formData.expected_final_date}
                  onChange={handleChange}
                />
              </div>
            </div>

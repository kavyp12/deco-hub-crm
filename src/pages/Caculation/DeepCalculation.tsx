  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col h-[calc(100vh-64px)] bg-gray-50">
        ...
        {/* Header */}
        <div className="bg-white border-b px-4 md:px-6 py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm z-20 w-full overflow-hidden">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" onClick={() => navigate('/calculations/edit/' + selectionId)} className="flex-shrink-0">
              <ArrowLeft className="h-5 w-5 text-gray-500" />
            </Button>
            <div className="min-w-0">
              <h1 className="text-lg md:text-xl font-bold flex items-center gap-2 text-gray-900 truncate">
                <FileText className="h-4 md:h-5 w-4 md:w-5 text-purple-600 flex-shrink-0" /> Deep Costing
              </h1>
          ...
          <div className="flex items-center justify-between self-end md:self-auto w-full md:w-auto gap-4 sm:px-0 px-2">
            <div className="text-left md:text-right">
              <span className="text-xs text-gray-500 uppercase font-bold tracking-wider block md:text-right">Grand Total</span>
              <div className="text-lg md:text-2xl font-bold text-purple-700">₹{grandTotal.toLocaleString()}</div>
            </div>
            <Button onClick={handleSave} disabled={saving} className="bg-purple-600 hover:bg-purple-700 h-10 px-4 md:px-6 shadow-sm flex-shrink-0">
              {saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />} <span className="hidden sm:inline">Save Calculation</span><span className="sm:hidden">Save</span>
            </Button>
          </div>
        </div>

        {/* TABS */}
        <div className="bg-white border-b px-4 md:px-6 py-3 flex flex-col md:flex-row gap-4 overflow-x-auto shadow-sm z-10 items-start md:items-center min-h-[60px] w-full">
          <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200 shrink-0 self-start md:self-auto w-full md:w-auto overflow-x-auto">
            {['Local', 'Roman', 'Forest', 'Somfy'].map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 md:px-4 py-1.5 text-xs font-bold rounded-md transition-all flex items-center gap-1 md:gap-2 flex-shrink-0 ${activeTab === tab ? 'bg-white text-purple-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                    }`}
                >
                  {tab}
                  {count > 0 && (
                    <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${activeTab === tab ? 'bg-purple-100 text-purple-700' : 'bg-gray-200 text-gray-600'
                      }`}>
                      {count}
                    </span>
                  )}
          ...
          <div className="hidden md:block h-6 w-px bg-gray-300 mx-2 shrink-0" />

          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
            <div className="flex items-center w-full gap-2 overflow-x-auto pb-1 md:pb-0">
              {/* BULK RATES */}
              <div className="flex items-center gap-2 text-xs font-bold uppercase text-gray-500 whitespace-nowrap shrink-0">
                <Settings className="h-4 w-4 shrink-0" /> <span className="hidden sm:inline">{activeTab} Rates:</span>
              </div>
              {Object.entries(bulkRates).map(([key, val]) => (
                <div key={key} className="flex-none flex items-center border border-gray-200 rounded bg-white overflow-hidden shadow-sm h-9">
                  <span className="flex-shrink-0 text-[10px] uppercase px-2 md:px-3 bg-gray-50 text-gray-600 font-medium border-r h-full flex items-center justify-center min-w-[40px] md:min-w-[60px]">{key}</span>
                  <Input className="h-full w-16 md:w-20 text-xs text-center border-0 focus-visible:ring-0 rounded-none px-1"
                    value={val} onChange={(e) => setBulkRates({ ...bulkRates, [key]: parseFloat(e.target.value) })} />
                  <button onClick={() => applyBulkRate(`${key}Rate`, Number(val))} className="flex-shrink-0 h-full px-2 md:px-3 bg-purple-50 text-purple-700 border-l"><Check className="h-4 w-4" /></button>
                </div>
              ))}
            </div>

            <div className="sm:ml-auto md:border-l sm:pl-4 flex gap-2 shrink-0 self-end sm:self-auto w-full sm:w-auto justify-end">
